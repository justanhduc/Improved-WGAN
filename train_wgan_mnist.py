...
from wgan_mnist import WGAN, DataManager2
...
def train(config_file, *args):
    ...
    if args[0] == 'wgan-gp':
        ...
        disc_cost += T.cast(net.LAMBDA, 'float32') * gradient_penalty
    elif args[0] == 'wgan':
    ...
    while epoch < net.n_epochs:
        ...
        for it in range(num_train_batches):
            ...
            if iteration % net.validation_frequency == 0:
                gen_images = test_gen()
                mon.save_image('generated image', gen_images)
                mon.flush()
            mon.tick()


if __name__ == '__main__':
    train('wgan_mnist.config', 'wgan-gp')
